...
ROOT_DATA_TEST_DIR = '/srv/data/jbang36/tasti_data/video_data/jackson17'
...
class ResizeModel(torch.nn.Module):
    ...
    def __init__(self, expected_height=12, expected_width=12):
        super().__init__()
        self.height = expected_height
        self.width = expected_width


    def forward(self, torch_data):
        #### we return a fixed size
        batch, channel, height, width = torch_data.shape
        height_skip = height // self.height
        width_skip = width // self.width
        skipped_data = torch_data[:,:,::height_skip, ::width_skip]
        ### now we probably need to flatten this data because that's expected
        skipped_data = skipped_data.reshape(batch, channel, -1)
...
class NightStreetOfflineIndexPixel(tasti.Index):
    ...
    def get_cache_dir(self):
        loc = '/srv/data/jbang36/tasti_data/cache/tasti_pixel'
    ...
    def get_target_dnn_dataset(self, train_or_test):
        if train_or_test == 'train':
            ...
            video_fp = os.path.join(ROOT_DATA_TEST_DIR, '2017-12-17')
        video = VideoDataset(
            video_fp=video_fp,
    ...
    def override_target_dnn_cache(self, target_dnn_cache, train_or_test):
        if train_or_test == 'train':
            ...
            labels_fp = os.path.join(ROOT_DATA_TEST_DIR, 'jackson-town-square-2017-12-17.csv')
        labels = LabelDataset(
            labels_fp=labels_fp,
...
class NightStreetOfflinePixelConfig(tasti.IndexConfig):
    def __init__(self):
        super().__init__()
...
if __name__ == '__main__':
    config = NightStreetOfflinePixelConfig()
    index = NightStreetOfflineIndexPixel(config)
    index.init()
