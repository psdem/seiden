...
from benchmarks.stanford.tasti.tasti.seiden.data.data_loader import LabelDataset  # ImageDataset,
from tqdm import tqdm
# import time
from src.motivation.tasti_wrapper import InferenceDataset


def calculate_top_dists(dataset_length, rep_indices, top_reps):
    """
...
def calculate_top_reps(dataset_length, rep_indices):
    """
...
def _get_closest_reps(rep_indices, curr_idx):
    result = []
...
def generate_matrix(images, random_indices):
    pixel_distances = []
    temporal_distances = []
    done = {}
...
def calculate_pixel_uncertainty(all_images, rep_indices, dataset_length):
    ...
    for i in range(dataset_length):
        ...
        left, right = _get_closest_reps(rep_indices, i)
...
def calculate_temporal_uncertainty(all_images, rep_indices, dataset_length):
...
def update_pixel_uncertainty(pixel_uncertainty, images_downsampled, left_rep, middle_rep, right_rep):
...
def update_temporal_uncertainty(temporal_uncertainty, images_downsampled, left_rep, middle_rep, right_rep):
    for i in range(left_rep + 1, right_rep):
        if i == middle_rep:
...
class EkoParameterSearch(Index):
    def __init__(self, config, images):
        self.debug = None
        self.f_uncertainty = None
        self.d_uncertainty = None
        self.beta = None
        self.alpha = None
        self.topk_dists = None
        self.topk_reps = None
        self.reps = None
        self.t_uncertainty = None
        self.images = images
    ...
    def calculate_rep_methodology(self):
        ...
        top_reps = calculate_top_reps(dataset_length, rep_indices)
        top_dists = calculate_top_dists(dataset_length, rep_indices, top_reps)
    ...
    def get_additional_anchors(self, rep_indices: list, n_reps):
        ...
        pixel_uncertainty = calculate_pixel_uncertainty(images_downsampled, rep_indices, dataset_length)
        temporal_uncertainty = calculate_temporal_uncertainty(images_downsampled, rep_indices, dataset_length)
        ...
        for _ in tqdm(range(n_reps - curr_size), desc="Choosing Other Rep Indices.."):
            ...
            left_rep, right_rep = _get_closest_reps(rep_indices,
            ...
            distance_uncertainty = update_pixel_uncertainty(pixel_uncertainty, images_downsampled, left_rep,
                                                                 chosen_rep, right_rep)
            temporal_uncertainty = update_temporal_uncertainty(temporal_uncertainty, images_downsampled, left_rep,
                                                                    chosen_rep, right_rep)
    ...
    def compute_alpha_beta(self, images, n_reps):
        ...
        x, debug = generate_matrix(images, random_indices)
    ...
    def generate_label_dist(self, images, random_indices, done):
        ...
        labels = self.override_target_dnn_cache(target_dnn_cache)
        car_counts = []
        for i in range(len(random_indices) - 1):
            j = i + 1
            idx_i = random_indices[i]
